...
declare -a checkpoints=(dense acdc_75)
...
python main.py \
	--dset=${transfer_dset} \
	--dset_path=${dset_path} \
	--config_path="./configs/mobilenet_full_net_transfer.yaml" \
	--workers=4 \
	--epochs=150 \
	--batch_size=64 \
	...
	--exp_name="${checkpoints[i]}-${transfer_dsets}" \
	--use_wandb \
	--wandb_name="${checkpoints[i]}-${transfer_dsets}" \
