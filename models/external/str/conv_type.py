...
def initialize_sInit():
    ...
    if True: #parser_args.sInit_type == "constant":
        return -3200*torch.ones([1, 1]) #parser_args.sInit_value*torch.ones([1, 1])
...
class STRConv(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        if True: #parser_args.sparse_function == 'sigmoid':
    ...
    def forward(self, x):
        ...
        sparseWeight = self.weight#sparseFunction(self.weight, self.sparseThreshold, self.activation, self.f)
        x = F.conv2d(
